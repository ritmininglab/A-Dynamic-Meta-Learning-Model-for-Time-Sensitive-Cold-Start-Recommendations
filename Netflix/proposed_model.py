    ...
    def forward(self, support_set_x, support_set_y, query_set_x, num_local_update,
        ...
        for idx in range(num_local_update):
            ...
            for i in range(batch_num):
                ...
                support_set_y_pred = torch.matmul(supp_xs,torch.mean(torch.stack([user_rep[0],rnn_hidden[0]]),dim=0).t())
        ...
        query_set_y_pred = torch.matmul(query_set_x,torch.mean(torch.stack([user_rep[0],rnn_hidden[0]]),dim=0).t())
...
if __name__ == "__main__":
    ...
    for period in range(1, 17):
        ...
        #Percentage RMSE computation
        rmse_result=[]
        idx_true = true_list.argsort()[::-1]
        tot_len = len(idx_true)
        ...
        time_rmse.append(rmse_result[-1])

    print('\n=======================\n')
    print('Average Netflix RMSE=',sum(time_rmse)/len(time_rmse))
    print('\n=======================\n')